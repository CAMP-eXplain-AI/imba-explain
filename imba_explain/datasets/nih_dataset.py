...
nih_cls_name_to_ind = {'Atelectasis': 0,
                       'Cardiomegaly': 1,
                       'Consolidation': 2,
                       'Edema': 3,
                       'Effusion': 4,
...
class NIHClassificationDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.img_to_label = {x: label_df[label_df['Image Index'] == x]['Finding Labels'].split('|')for x in img_files}
    ...
    def __getitem__(self, idx: int) -> Dict[str, Union[Tensor, str]]:
        ...
        label = torch.LongTensor([nih_cls_name_to_ind[l] for l in label])
